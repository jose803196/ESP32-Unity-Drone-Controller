...
// Nueva bandera para controlar el envío de datos al controlador
...
// Callback para recibir datos de los sensores y enviarlos al controlador
static void espnow_recv_cb(const uint8_t *mac_addr, const uint8_t *data, int len) {
    if (len == sizeof(SensorData)) {
        SensorData received_data;
        memcpy(&received_data, data, len);
        
        // Enviar datos al UART para Unity
        char buffer_tx[256];
        ...
        // Enviar datos al controlador si sendingEnabled es true
        if (sendingEnabled) {
            esp_now_send(mac_controlador_emisor, (uint8_t*)&received_data, sizeof(SensorData));
            ESP_LOGI("PUENTE", "Datos enviados al controlador");
        }
    }
}

// Tarea para leer comandos de Unity y controlar el envío
void uart_task(void *pvParameters) {
